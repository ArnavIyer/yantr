...
bool VescInterface::connect(const std::string& port) {
  ...
  if (isConnected()) {
    throw std::runtime_error("Already connected to VESC on port " + port);
  }
  ...
  int result = pthread_create(&rx_thread_,
                              ...
                              NULL);
  if (result != 0) {
    throw std::runtime_error("pthread_create failed with error code " + std::to_string(result));
  }
  return true;
}
...
void VescInterface::send(const VescPacket& packet) {
  ...
  if (static_cast<size_t>(written) != packet.frame().size()) {
    throw std::runtime_error("Wrote " + std::to_string(written) + " bytes, expected " + std::to_string(packet.frame().size()) + ".");
  }
}
